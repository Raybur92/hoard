  ...
  return (
    <div style={{ width: w, flex: '0 0 auto' }}>
      <div style={{ position: 'relative' }}>
        <Cover w={w} h={h} label={g.title.toUpperCase()} dev={g.developer} year={shortYear(g.year)} bright={g.progress > 0} />
...
function Shelf({ idx, shelf }: { idx: number; shelf: ShelfDisplay }) {
  ...
  const shown = shelf.items.slice(0, 8);
  ...
  return (
    <div style={{ padding: '24px 0' }}>
      <div className="shelf-label">
        ...
        <span className="total" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
          scroll <Icon name="arrowR" size={11} />
        </span>
      </div>
      <div style={{ display: 'flex', gap: 16, overflow: 'hidden', position: 'relative' }}>
        {shown.map(g => <ShelfItem key={g.id} g={g} isBacklog={isBacklog} />)}
        {remaining > 0 && (
          <div style={{ width: 130, flex: '0 0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', border: '1px dashed var(--rule-bright)', height: 174, color: 'var(--paper-faint)', fontSize: 11, gap: 6 }}>
            <span style={{ fontSize: 22 }}>+{remaining}</span>
