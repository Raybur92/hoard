...
**T1 done** (migration + types). **T2 done 2026-05-08** (PSN trophy fetcher + orchestrator + auto-complete helper). T4 was extracted alongside T2 because T2 needed it. PSN syncs now pull trophy aggregates inline, populate `Game.psnNpCommunicationId` on first match, and auto-flip eligible UserGames to `Completed` at 100%. **Next phase: T3 — Steam achievement fetcher** (per-game on background queue, throttled). Per hard rule 10, agent stops after T2 and waits for Andrea's go-ahead before T3.
