...
- **R3** ✅ Done 2026-05-07. New `ReleasesDesktop` composing R2 primitives with full URL state (`?mode=&scope=&zoom=&bucket=`), bucketing logic per D4 (6 month buckets / 3-quarter+TBA), right-rail conditional, empty states with `[skip ahead]`. `/upcoming` → `/releases` redirect, sidebar/tab-bar relabeled, `UpcomingDesktop` deleted. Stub `/releases/recent` route until R4 lands. 31 new bucketing tests.
- **R4** ✅ Done 2026-05-07. `ReleasesRecentDesktop` extracted from the R3 stub into its own file. Two-section layout per handoff §10: `// just out · starred · N` and `// also released · not on your wishlist · N` (each a 2-col grid). Green prompt strip at the top of the content area mirrors the green banner styling but is informational only (no action button). Empty state ("// nothing in the last 14 days" + back-to-releases CTA) and dedicated error/retry path. Drift-guard test asserts no `[i got it]` / `[mark all owned]` buttons. 5 new Vitest smoke tests.
- **R5** ✅ Done 2026-05-07. Full mobile shell rewrite. New primitives `MobileViewHeader` (chevron stepping + tappable label that opens the sheet, `disabled` at bucket extremes per handoff §8 + §12 punch-list 10), `MobileViewSheet` (single mobile surface for mode/scope/zoom/bucket — applies on every dismissal path per §7; scope conditional on mode=all; zoom change re-maps bucket via local helper), `MobileBanner` (compact 1-line variants per §12 punch-list 3+4), `MobileReleaseRow` (40/36/1fr/auto grid per §12 punch-list 2). New `ReleasesMobile` page composes them with full URL-state parity to desktop; tab-bar SOON nav lands on `/releases` (no params) so defaults hydrate per §3. New `ReleasesRecentMobile` uses existing `MobileHeader` + asymmetric two-section layout (panel cards for starred, list rows for non-starred per §10). `UpcomingMobile.tsx` deleted; route mapping + screen index updated. 20 new Vitest tests covering chevron disabled state, sheet apply paths, scope-preservation across mode toggle, banner conditional logic + drift-guard.
