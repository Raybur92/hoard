...
- **PR B — Real wishlist scope + Path-B persistence fix** ✅ Done 2026-05-06. `?scope=wishlist` on `/api/igdb/upcoming` reads persisted `WishlistRelease` rows (no IGDB round trip, no hype filter); the wishlist-toggle endpoint persists `releaseDate` / `platforms` / `synopsis` / `hype` / `category` / `releaseDateCategory` instead of dropping them. New `WishlistRelease.category` column. Backfill re-fetched 7/7 existing impoverished rows. Frontend: 3 real scope chips with honest wishlist count.
- **PR C — Sync-all + wipe-library** (next) with proper microinteraction feedback.
