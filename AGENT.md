...
The smart matcher (#32) prevents wrong-game imports going forward; `scripts/audit-mismatches.ts` surfaces platform-collision-class mismatches in existing data; but the wrong-sequel class (Slay-the-Spire-2 instead of Slay-the-Spire) is detectable only by the user. Fix path: a `[wrong game?]` chip on GameDetail (desktop + mobile) opens `RemapGameModal` — an IGDB search dialog pre-filled with the current title — and the user picks the right one. Server-side `POST /api/games/:userGameId/remap` upserts the new `Game` row and rewrites `UserGame.gameId`, **preserving notes, status, playtime, rating, addedAt, lastPlayedAt**. `(userId, gameId)` unique-constraint collisions return 409 instead of throwing. **Decision rejected:** automated re-resync that rebinds existing UserGames to corrected matches. To do that reliably we'd need a platform-side stable identifier on each UserGame (PSN's `titleId`, Steam's `appid`) — we only have `steamAppId` on the `Game` row, not per-UserGame. Storing platform IDs per UserGame is a v2 schema-change effort that doesn't retroactively help current bad rows (those don't have the platform IDs stored). The remap chip + audit script combo is a complete user-driven fix loop with no schema cost.
