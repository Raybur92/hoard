...
- **NEW (per strategy):**
  ...
  - `mediaType` (`DIGITAL | PHYSICAL`) — default `DIGITAL`. *Per S7 / S11.* Simplified 2026-05-22 (Andrea): disc-vs-cart-vs-ROM distinction was over-modeling. The platform code already tells you the physical form (SNES = cart; PS5 = disc; emulator setup = ROM). `mediaType` just needs to differentiate "I bought it digitally" from "I have a physical copy."
...
| **Owned (manual)** | Same statuses AND (`mediaType = PHYSICAL` OR no playtime data) | "What I own" — no sync data; honest about missing fields |
...
| **Wishlisted, released — physical** | `status = Wishlist` AND past release AND `mediaType = PHYSICAL` | "Why I want this + where to find a copy" — gallery, news, reviews, current marketplace **listings** (§3.19), region/condition filters, HLTB |
...
| **Media** *(of a UserGame)* | Format, Carrier | The physical/digital form. Two values: `DIGITAL` · `PHYSICAL`. Simplified 2026-05-22 — disc/cart/ROM distinction was over-modeling; platform tells you the physical form. New per S7 / S11. |
...
**`MediaType`** *(new, horizon-2; simplified 2026-05-22)* — `DIGITAL` · `PHYSICAL`. The disc/cart/ROM distinction is inferable from `platformLabel` (SNES = cart, PS5 = disc, emulator = ROM); the mediaType field just needs to differentiate digital purchases from physical copies. `MarketplaceListing.mediaType` (§3.19) keeps the richer enum because secondhand listings need that granularity for catalog search.
...
- **CM13 — Wishlist auto-promotion to library on ownership detection.** 2026-05-22 (Andrea — clarified F1-PR2 scope). When an incoming sync row OR a manual-add operation matches an existing `UserGame(status='Wishlist')` for the same game, the status auto-flips: incoming playtime > 0 → `OnHold`, otherwise → `Backlog`. The trigger condition is **specifically** `status == 'Wishlist'` — any other status is preserved (the user's manual library state survives auto-promotion naturally because the trigger doesn't fire). Companion `WishlistRelease` row (when present from the Releases-page upcoming tracker) is **NOT** touched — per Andrea: "two separate logics." The Releases-page tracker has its own lifecycle (release date passes → tracker becomes stale naturally; not actively cleaned by the auto-promotion path).

  This formalises a behavior that was implicitly missing: previously a sync that found a game already in the user's library with status=Wishlist would either no-op the status (current behavior pre-2026-05-22) or fail to merge cleanly. The auto-promotion makes the experience "I wishlisted GTA, then bought it on Steam, then synced → it's now in my library at Backlog/OnHold automatically" instead of "still on the Wishlist shelf with phantom playtime."

  Implementation lives in a shared helper (`promoteWishlistOnOwnership` in `apps/api/src/services/`) called by both `syncRunner.ts` and the manual-add route. Logic is platform-agnostic — it doesn't matter which platform the new ownership arrived from; the global `status` flips when ANY ownership is detected. Naturally co-locates with the F1-PR5 backend-upsert work, but small enough (~30 lines) to bundle with F1-PR2 if shipping the schema + auto-promotion together feels cleaner. Andrea's call at PR-slicing time.

  **CM13 amendment (2026-05-27, P-series).** Engagement signal extended beyond playtime. Sony's `getUserPlayedGames` (and Steam's `playtime_forever`) lag 24-72h on new releases — a user can wishlist a game, play it for hours, earn trophies, and `getUserPlayedGames` still reports `playDuration: 'PT0S'`. CM13's original `playtime > 0 → OnHold` rule would never fire on these. Sister helper `promoteWishlistOnEngagement(status, earned, percent)` in `apps/api/src/lib/achievements.ts` fires on trophy/achievement evidence: any `earned > 0` flips Wishlist → OnHold (or Completed at 100% — folds in T-D2 for the wishlist case). Wired into `applyPsnTrophyAggregates` (PSN trophies), `triggerSteamAchievementsBackground` (Steam achievements, with candidate filter widened to include Wishlist UserGames whose Game has a `steamAppId`), and `applyXboxPlaytimeBackground` (re-evaluates Wishlist when the side-pass surfaces real playtime post-syncRunner). The trigger remains `existingStatus === 'Wishlist'` — non-Wishlist states are still preserved. Conceptually: CM13 isn't about "ownership detection via playtime" but "ownership detection via any engagement evidence," with playtime + trophies + achievements all qualifying. The original `promoteWishlistOnOwnership` still owns the playtime path; the new `promoteWishlistOnEngagement` owns trophy/achievement paths. Both share the same outcome rules.

  Amends CM12 default behavior: when wishlisting from any surface (Releases-page toggle, manual-add modal with status=Wishlist, GameDetail [+ wishlist]), the resulting `UserGame.wishlistedPlatforms` is **empty by default** (no IGDB-population). Per-platform wishlist binding is an opt-in collector affordance accessed via GameDetail (explicit "I want this on PC" tap), not an automatic surface. This reverses the earlier OQ-S-13 lock (which would have auto-populated wishlistedPlatforms from IGDB) — the auto-promotion makes the default-empty case the load-bearing one because most users want generic "I want this game" intent, not platform-specific.
