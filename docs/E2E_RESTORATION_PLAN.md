...
| 5 — E2 (welcome.integration.spec.ts — single integration spec) | **Done (2026-05-21)**                                                                  | 4-commit chain: `cb1876a` (docs §E2 expansion), `1ce96ec` (seed-e2e.ts +5 InviteCode rows), `6ef2e0f` (welcome spec + globalSetup + playwright.config wiring), `dbaced4` (quarantine test 5 + fixture cleanup defense). Plus one Flag-A fix-up `1c4d2d9` (project-level testIgnore replaces runtime skip — eliminates the `6 skipped` mobile-project noise). End state: **5/6 welcome tests passing** (tests 1-4 + test 6); **test 5 quarantined under #6** pending diagnosis of the UserProvider `loading → authed` mount race where the redeem-button click is lost before reaching the server (verified working at the product layer via direct `/api/auth/me` calls returning `hasRequestedAccess: true` with byte-identical cookies — failure is in Playwright observation timing, not product correctness). Verification §E2.4 all 5 checks green: full-suite shape 41 passed / 1 skipped / 2 pre-existing Dashboard flakes (Check 1e); ghost-purge fires on >1h ghost spares <1h (Check 2); 0 ghost accumulation after 2 back-to-back welcome runs (Check 3); globalSetup fail-loud with named-paths error on bad `DATABASE_URL_TEST` (Check 4); InviteCode pool intact — codes 1 + 2 SetNull-restored with usedAt timestamps preserved, code 3 unchanged from seed, codes 4 + 5 untouched, zero admin-generated stragglers (Check 5). Andrea-led decisions during the workstream: D4 (drop component spec — folded into integration test 6), Fix 1 (fixture cleanup always re-logs in as tracked email before DELETE), Fix 2 (explicit status+body capture on `codeRes.ok()` failures — caught Zod 100-char `note` cap), Flag A (project-level `testIgnore` over runtime `testInfo.skip`), the two-context structural refactor for test 5, and finally the (g) quarantine + diagnostic-trail-as-issue path when the structural rework didn't resolve the failure.                                                                                                                                                                                                                                                              |
