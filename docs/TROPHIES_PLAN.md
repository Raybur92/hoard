...
| T2 — PSN trophy fetcher | Done (2026-05-08) | New `getPsnTrophyTitles(npssoToken)` in `apps/api/src/services/platforms/psn.ts` — paginated `getUserTitles`, returns the cleaned shape with `npCommunicationId` + per-rank earned/defined trophy counts + `lastUpdatedAt`. New orchestrator `applyPsnTrophyAggregates(userId, trophyTitles)` in `apps/api/src/services/trophies.ts` — pre-builds two lookup Maps over the user's library (by `psnNpCommunicationId`, by normalized title for fallback), iterates trophy titles, persists `Game.psnNpCommunicationId` on first title-fallback match, updates the four `UserGame.achievements*` columns, applies T-D2 auto-complete via shared `applyAutoCompleteRule` helper. Wired into `routes/platforms.ts` PSN branch — runs after `runSync` returns; trophy-fetch failures don't fail the whole sync (library import already succeeded). T4 helper extracted early because T2 needed it. **184 API + 193 web tests pass** (15 new — 9 helper × matrix, 11 orchestrator on matching/auto-complete/aggregate math + 2 on normalize). Lint + typecheck + rename-rule clean. |
| T3 — Steam achievement fetcher | Done (2026-05-08) | New `apps/api/src/services/platforms/steamAchievements.ts`. `getSteamAchievementsForGame(steamId, appid)` calls `ISteamUserStats/GetPlayerAchievements/v1` once per game; returns `{ earned, total } \| null` where `null` covers private-profile / unsupported-game / HTTP-error / network-fail / empty-array silently (T-D7). `triggerSteamAchievementsBackground(userId, steamId)` filters the user's library to UserGames that have BOTH a `Game.steamAppId` AND a Steam-platformed playtime entry, throttles at ~3 req/s (330ms `delay`), writes the four `achievements*` columns, applies T-D2 auto-complete via shared `applyAutoCompleteRule`. Per-game errors don't kill the loop. Wired into `routes/platforms.ts` Steam branch as a fire-and-forget — `runSync` returns immediately, `Platform.syncStatus` flips to `ok` so the UI is responsive, and achievement data trickles in over the next few minutes. 14 new tests cover the fetcher's five "no data" branches + the orchestrator's filter logic + write/auto-complete/error paths. **198 API + 193 web tests pass.** Lint + typecheck + rename-rule clean. |
| T4 — Shared auto-complete helper | Done (2026-05-08, alongside T2) | `applyAutoCompleteRule(currentStatus, percent)` extracted to `apps/api/src/lib/achievements.ts` ahead of schedule because T2 needed it. T3 will reuse it as-is. 9 tests cover the full status × percent matrix. |
| T5 — GameDetail receipt UI + Steam scope-tab note | Done (2026-05-08) | New `achievementLabel(playtimeByPlatform)` helper in `apps/web/src/lib/utils.ts` returns `'trophies'` when the user has any PSN playtime, `'achievements'` otherwise (Andrea is PSN-heavy so dual-platform games default to "trophies"). `GameDetailDesktop`: inline-marker line `// trophies · 12/35 · 34%` rendered after the HLTB block, hidden when `achievementsTotal === null`, percent colored green when ≥ 80 / paper-dim otherwise. `GameDetailMobile`: same data woven into the existing PROGRESS pre-block as a dotted-row matching the HLTB rows. T-D7 amendment: `PlatformDetailDesktop` and `PlatformDetailMobile` scope tabs now render a Steam-only "// note · steam profile must be public for achievement sync. settings → privacy → game details on steam." subline directly under the existing `trophies` checkbox. `ScopeTab` desktop component gained a `code` prop. 3 new label-inference tests. **198 API + 196 web tests pass.** |
| T6 — Dashboard rollup | Done (2026-05-08) | `DashboardStats.achievementsRollup: { earned, total, percent } \| null` returned from `GET /api/dashboard`. Single Prisma `aggregate` query with `_sum` on `achievementsEarned` + `achievementsTotal` scoped to userId AND filtered to `achievementsTotal: { not: null }` so unsupported games don't pollute the denominator. `null` rolls up when no row in the user's library has any achievement data — UI hides the line entirely instead of rendering `0/0 · NaN%`. `DashboardDesktop` surfaces it with a `Gauge` block matching the existing completion-ratio gauge for visual symmetry. `DashboardMobile` surfaces it as a single bordered row in the receipt aesthetic. Percent goes green at ≥ 80, paper-dim otherwise (matches T5). 4 new dashboard tests cover null/0%/aggregate-shape paths. **202 API + 196 web tests pass.** |
