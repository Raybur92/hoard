...
- **Stage 1: "which ecosystem?"** — six buckets by manufacturer / family: **PC · PlayStation · Xbox · Nintendo · Sega · Other**. Restructured 2026-05-22 (Andrea's smoke-test pushback): the original Digital / Physical / Retro buckets conflated stores with consoles (Digital listed stores like Steam/PSN/eShop, Physical/Retro listed consoles like PS5/Switch/SNES). The corrected mental model: a game is published on consoles; stores are sync metadata Hoard handles automatically. Picker contents are CONSOLES ONLY. Stores (Steam, PSN, Xbox Live, eShop, GOG, Epic, Itch, Humble) fold into the relevant console family — PC for PC storefronts; PSN-digital purchases get recorded under PS5/PS4 directly. When the user just picked an IGDB result in P1, the picker pre-opens the bucket matching IGDB's first reported platform (Pokemon Red → IGDB reports Game Boy first → Nintendo bucket pre-opens; GTA V → IGDB reports PS5 first → PlayStation bucket pre-opens).
...
[ content: inline summary "// added {title} · {platform} · {status}" + three CTAs ]
  ...
  - 15s auto-close timeout (no user action)       → close modal; user is back at origin
...
| Edge | Behavior |
|---|---|
| 15s auto-close, no user action | Modal closes silently. Cache already invalidated server-side at save; library shelves already reflect the new entry. |
...
| Code | Resolution |
|---|---|
| ~~**OQ-F1-1**~~ | ~~Two-stage platform picker vs. flat searchable~~ — **RESOLVED → two-stage with smart defaults.** Stage 1: **six manufacturer buckets** (PC · PlayStation · Xbox · Nintendo · Sega · Other), PC opened by default. Originally three buckets (Digital · Physical · Retro) — restructured 2026-05-22 (Andrea's smoke-test) because that bucketing conflated stores with consoles. New structure: consoles only, bucketed by family. Stage 2: within-bucket list, type-to-filter, recently-used pinned at top (bulk-add collector use case). Rationale: maps to user cognitive structure (collectors think "my Nintendo collection / my Sony collection"), avoids 50-option overwhelm, recently-used pinning serves the bulk-add S7 use case. |
...
| ~~**OQ-F1-9**~~ | ~~IGDB-suggested platforms~~ — **RESOLVED 2026-05-22 → yes.** When user picks an IGDB result in P1, P2's picker pre-opens the bucket containing the FIRST IGDB-reported platform (PC / PlayStation / Xbox / Nintendo / Sega / Other per the post-restructure bucket scheme), and pins the IGDB-reported platforms at the top of stage-2 (above the recently-used pin section). Pure UX win; IGDB data already fetched. Folds into F1-PR1. |
...
[ content: inline summary "// added {title} · {platform} · {status}"
           · three CTAs · 15s auto-close timer ]
...
- 15s auto-close (no interaction)               → close modal; user back at origin
- Esc / click outside                          → close modal
...
| Code | Question | Resolves in |
|---|---|---|
| **OQ-F1-1** | Two-stage platform picker (manufacturer buckets — PC / PlayStation / Xbox / Nintendo / Sega / Other) vs. single big searchable list — depends on whether users prefer the structured drilldown or the speed of a flat search. | Paper-prototype probe in Phase 4 walkthrough OR deferred to surface-layer design. |
