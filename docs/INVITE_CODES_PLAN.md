...
| I5 — Admin page + sidebar entry | **Pending** (awaits Andrea's smoke #3 retry against `9051b36`) | Desktop-only `/admin` per I-D3; sidebar entry conditional on `isAdmin` per I-D2; mobile fallback "use a desktop browser" message. Three sections: PENDING ACCESS REQUESTS (with prefilled-note generate-code button per row), ALL USERS (terminal-style aligned table), INVITE CODES (with `[revoke]` on unused rows). `[+ GENERATE CODE]` modal with copy-to-clipboard + green `// copied` toast. Hooks: `useAdminUsers()`, `useAdminInviteCodes()` over the SWR cache. New `AdminScreen.test.tsx` + `admin.spec.ts` E2E (the latter contingent on E1's strategy decision since admin tests can't seed code data without DB access). |
| I6 — Rollout + verification + doc closeouts | **Pending** (awaits I5) | Generate first batch of real codes via `/admin`, verify with one friend (Marco per spec §8), 24-hour log-monitoring window for stray `403 PENDING_INVITE` on active users, doc closeouts across PLAN/CLAUDE/AGENT/INVITE_CODES_PLAN. |
