...
| M2 | Epic auto-sync | Done 2026-05-28 | `f2d22ed` + `7d9d9e8` + `b0eb002` |
| M3 | Nintendo auto-sync | Done 2026-05-29 | `2bb75f0` (backend WIP) + this commit |
| M-doc | Hard Rule #6 amendment + AGENT.md decision + memory updates | Done 2026-05-29 (this commit) | — |
...
**M3 closeout notes (2026-05-29, this commit + the WIP backend in `2bb75f0`):**
- `Game.nintendoTitleId String? @unique` added (Switch application IDs are 16-character hex strings — opaque, not numeric; same shape as Epic's catalogItemId). Migration `20260528170000_nintendo_title_id` applied via `prisma db execute` + Node `$executeRaw` migration-record insert. The CLI `prisma db execute` and `prisma migrate resolve` were the canonical recipe (CLAUDE.md operational gotchas); on this run `db execute` was silent on success (verified post-hoc via `information_schema.columns`) and `migrate resolve` was bypassed by the Node fallback to avoid the documented advisory-lock hang.
- Backend (landed in WIP commit `2bb75f0`, ~462 lines): `apps/api/src/services/platforms/nintendo.ts` implements the full Nintendo Account OAuth + PKCE flow for the Parental Controls "Moon" API. Functions: `generateNintendoPkce` (RFC 7636 verifier + S256 challenge), `getNintendoAuthUrl`, `extractSessionTokenCode` (handles full `npf...://auth#session_token_code=...` redirect URLs, query-string variants, and bare-code paste), `exchangeNintendoSessionTokenCode` (one-shot code → long-lived session_token), `exchangeNintendoAccessToken` (session_token → 15-min access_token; uses Dalvik UA — Nintendo gates on UA at this endpoint), `ensureFreshNintendoCredentials` (identity-equality refresh pattern from GOG — caller checks `fresh === creds` to skip DB write), `getNintendoAccountUser` / `getNintendoUsername` (NA profile via api.accounts.nintendo.com, NASDKAPI UA — different from the Moon API base), `getNintendoDevices` (v2 fetchOwnedDevices), `getNintendoLatestMonthlySummary` (per-device playtime aggregation; returns null on 404 — newly-paired consoles take ~24h before Nintendo's first daily summary lands), `syncNintendoLibrary` (orchestrator: list devices → per-device monthly summaries → aggregate per-applicationId playtime + max(lastPlayedAt) across players + devices). Moon API version constants (ZNMA_VERSION='2.4.0' / BUILD='660' / Android 14) mirror pynintendoparental's `const.py` — Nintendo bumps the floor every ~2-3 months; refresh from https://github.com/pantherale0/pynintendoparental/blob/main/pynintendoparental/const.py when sync starts returning `errorCode=update_required`.
- IGDB resolution: `getGameByNintendoTitleId` in `apps/api/src/services/igdb.ts` wraps the shared `getGameByExternalUid('nintendo.com', uid)` from N-FIX. syncRunner cascade extended: `steamAppId → psnConceptId → xboxTitleId → gogAppId → itchGameId → epicCatalogItemId → nintendoTitleId → title-search → localization`. P2002 collision recovery extended to handle nintendoTitleId.
- Routes (in `2bb75f0`): `GET /api/platforms/nintendo/auth-url` (generates PKCE pair, returns `{ url, verifier, state }`); `POST /api/platforms/nintendo/connect` (accepts `{ redirectUrl OR code, verifier }` — extracts the session_token_code, exchanges through the auth chain, fetches the NA profile, persists `{ sessionToken, accessToken, naId, expiresAt, username? }` on Platform.credentials, returns 400 on Nintendo rejection BEFORE any DB write so a bad code → no half-row); NT branch in `POST /platforms/:code/sync` (uses `ensureFreshNintendoCredentials` with the GOG-style merge-on-refresh persistence; access_token is refreshed every ~15 min via session_token); NT added to all `validCodes` lists across sync/log/patch/credentials endpoints; NT in username backfill block via `getNintendoUsername`; NT in the `/credentials` reveal endpoint exposes `naId` only — `sessionToken` is too sensitive to expose.
- Frontend (this commit): new 7-step `NintendoGuidedFlowDesktop` + `NintendoGuidedFlowMobile` lazy-routed at `/settings/platforms/nt/connect`. Step sequence: (1) explainer + parent/child pairing diagram, (2) install Parental Controls app (App Store + Play Store deep links), (3) pair Switch with own Nintendo Account as parent, (4) open auth URL — desktop renders a QR code (220×220 white-on-transparent, via `qrcode` npm package) for the desktop-to-mobile sign-in handoff, mobile just shows the tap-to-open link, (5) copy the redirect URL after sign-in (with "site can't be reached" framing — that's expected, the address bar still has the code), (6) paste the URL into Hoard (`extractCode` regex handles full npf:// URLs / query-string variants / bare codes), (7) all set. Skip-ahead link on steps 2 and 3 for returning users who've already paired. Auth URL + verifier fetched lazily on first reach of step 4 — the PKCE pair doesn't sit around if the user gets stuck on the explainer. `api.nintendoAuthUrl()` + `api.connectNintendo({ redirectUrl, verifier })` added to the client with cache invalidation. PlatformDetailDesktop + PlatformDetailMobile + SettingsDesktop + SettingsMobile all flipped NT from `syncable: false → true` / dropped the `unsupported` branch. `qrcode` + `@types/qrcode` added to apps/web devDeps (~17 transitive packages); production bundle adds ~41 KB / ~14.6 KB gzipped as `NintendoGuidedFlowDesktop-*.js` (lazy-loaded, not in initial JS).
- Tests: +32 backend `nintendo.ts` (PKCE + extract + 3 exchange paths + ensureFresh × 2 + NA profile × 2 + Moon devices × 3 + monthly summary × 2 + sync × 7) + 11 platforms route (auth-url × 2 + connect × 7 + credentials reveal × 2) = +43 backend = **39 backend suites / 578 tests passing** (+44 from M2's 534). +6 frontend `NintendoGuidedFlow` (lazy fetch / link + QR / error / skip-ahead / save happy / save error) + 1 api-invalidation. Typecheck + lint + rename-rule guard + production build all clean.
- M-D14 (Hard Rule #6 amendment) landed in this commit: CLAUDE.md Rule #6 rewritten from "Nintendo and Epic are manual-only" to "Manual-add via IGDB search + `platformLabel` is the FALLBACK for any platform without an auto-sync path. All 7 platforms now sync; new platforms get added via the M-series pattern."
- Q1 from §5 resolved during the WIP probe spike — the Moon API monthly_summaries response includes a `shopUri` per title that points at `nintendo.com/store/...` URLs, BUT IGDB's external_games table for Switch titles is sparse. M3 stores `nintendoTitleId` (the opaque 16-char applicationId) as the stable join key and uses `getGameByExternalUid('nintendo.com', titleId)` — falls through cleanly to title-search + localization for the majority of rows. Match-rate caveat same as itch (M-D9): sparse IGDB coverage for Switch titles; manual-add still works for the long tail.
- Q4 from §5 (Parental Controls parental-account self-registration) confirmed during the live probe: a single Nintendo Account can pair as both parent and child of its own console. UX caveat captured in the guided-flow copy: step 1 frames it as "you'll act as your own guardian."
- One sticky property worth knowing for the next M-class platform: Nintendo's npf:// custom redirect scheme means the browser shows a "this site can't be reached" error after sign-in. The first round of UX copy hand-waved this as "complete sign-in" and confused testers — the version that shipped is explicit: "your browser will show a page-not-found error. that's expected — the address bar still has the code." When the next platform uses a custom URL scheme redirect, mirror the pattern.

**Pre-deploy operational gotcha:** Moon API version constants (`ZNMA_VERSION`, `ZNMA_BUILD`, `ANDROID_OS_VERSION`, `DEVICE_MODEL`) gate every Moon endpoint request. Nintendo bumps the floor every ~2-3 months; when sync starts returning `errorCode=update_required` or 401s clustered around a date, refresh from pynintendoparental's const.py and redeploy. **Quarterly maintenance tax** — captured as M3's first AGENT.md decision (#43) because this is architectural-shaped (it constrains how M-series platforms are kept alive).

**Post-deploy hotfixes (2026-05-29) — two bugs the probe spike missed:** Andrea's live smoke test against his real Switch surfaced two distinct 403 forbidden failures on `fetchOwnedDevices`. Both shared a root cause: the WIP probe (`scripts/probe-nintendo-moon.ts`) verified only the Nintendo Account OAuth chain (session_token_code → session_token → access_token → NA `/users/me`) but never actually called any Moon endpoint. The probe's claim "this works end-to-end" was only true for the auth chain, not for the Moon API itself.

- **Hotfix 1 (`3393fd6`) — scope list.** The WIP's 5-scope subset (`openid`, `user`, `moonDailySummary`, `moonMonthlySummary`, `moonUser:administration`) was too narrow. Replaced with the canonical 13-scope set from pynintendoauth's `KNOWN_NINTENDO_SERVICES["54789befb391a838"].scopes`. The critical missing scope was `moonOwnedDevice:administration` — exactly what `fetchOwnedDevices` authorizes against. The "Nintendo no longer grants the full 13-scope list" comment in the original code was a misdiagnosis from the probe — pynintendoparental ships with the full list and it works.

- **Hotfix 2 (`2b9967f`) — id_token vs access_token bearer.** Even after Andrea re-authed against the 13-scope auth URL, Moon still returned 403. The real bug: **Nintendo's Moon API validates the `id_token`, NOT the `access_token`.** Verified against pynintendoauth's `access_token` property which returns `f"Bearer {self._id_token}"` — confusingly named, but the wire behavior is clear: every Moon request uses the id_token as the Authorization bearer. The NA-account API at `api.accounts.nintendo.com` IS access_token-bearer (which is why `getNintendoAccountUser` worked and Andrea's nickname came back). The Moon API at `app.lp1.znma.srv.nintendo.net` is id_token-bearer. Two endpoints, two different tokens. Added `idToken` to `NintendoCredentials`, made `moonHeaders` / `getNintendoDevices` / `getNintendoLatestMonthlySummary` / `syncNintendoLibrary` all take/use idToken instead of accessToken, and made `ensureFreshNintendoCredentials` force-refresh when `idToken` is missing (the migration path for the legacy Platform row Andrea already had — first sync after deploy pulled a fresh pair, no disconnect/reconnect needed).

**Sticky property — probe spikes MUST hit the real target endpoint.** The original probe stopped at "Nintendo issued an access_token, NA `/users/me` accepted it, nickname came back." Both the scope correctness AND the bearer-token-type correctness were unverified. Production smoke-test surfaced both in 30 minutes; the cost of two re-deploys was an hour. **Future probe spikes for new platform integrations:** end the probe by calling a representative real-target API endpoint (Moon `fetchOwnedDevices`, GOG `/account/getFilteredProducts`, Epic `library/api/public/items`, etc.) — OAuth correctness ≠ API correctness. Mirrored as AGENT.md decision #44.

**Live end-state on Andrea's PSN sync (2026-05-29 15:22):** `sync ok in 2.3s`, Moon API authoritative, library count pending (newly-paired console; Nintendo's documented 24h post-pairing wait before the first daily summary lands; check again 2026-05-30 same time).
