...
- **Post-add edit affordances on GameDetail (manual playtime + collector metadata)** — F1-PR3 only seeds `playtimeByPlatform[platformLabel]` from the add-time `manualPlaytimeMinutes` field. There's no way to enter or correct playtime for a game already in the library, and the same gap applies to mediaType / condition / region (entered at add-time, no edit affordance after). **Locked constraint (Andrea 2026-05-23):** the manual playtime editor on GameDetail must be **available only for non-synced platform rows** — i.e., platform code NOT in the synced set `{ST, PS, XB, GG}`. Synced platforms have the sync as source-of-truth; allowing manual edits there would race with the next sync. For UserGames with mixed-source playtime (e.g., Steam-synced ST row AND a manually-added Game Boy row on the same game via smart-matcher merge), the editor is per-row: editable on the manual row, read-only on the synced row. Same per-row gating likely applies to mediaType / condition / region when those become editable.
