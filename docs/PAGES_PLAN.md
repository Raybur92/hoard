...
- **B-IGDB-3 (foundation + Dashboard 3-tab + Library chip strips + primary-lens routes + browse-by panel — fully done as of B-IGDB-3b2)** IGDB-tag triple — **genres + themes + perspectives** — as three distinct filterable / browseable dimensions across the library. IGDB exposes all three as separate axes on every game:
    ...
    **B-IGDB-3 (partial) landing notes 2026-05-30.** Foundation + Dashboard consumer shipped; Library consumer (filter chip strips + primary-lens routes + browse-by panel) split into a follow-up **B-IGDB-3b**.
    - **Shipped:** `Game.themes String[]` + `Game.playerPerspectives String[]` schema columns (migration `20260530120000_game_themes_perspectives`, defaults `[]`, hand-written SQL with `ADD COLUMN IF NOT EXISTS` for prod idempotency, applied via the documented `prisma db execute` + `migrate resolve --applied` recipe). IGDB service factories (`mapToSearchResult` + 3 `IgdbUpcomingRelease` factories + `getReleaseDetails`) extended to fetch + map both axes; query strings include `themes.name, player_perspectives.name`. All `Game.upsert` call sites updated: `syncRunner` (the main path), `wishlistImport`, `routes/upcoming` (wishlist toggle), `routes/games` (remap). `WishlistRelease` itself NOT extended — its snapshot doesn't carry themes/perspectives because Releases cards don't display them today; the wishlist-scope branch + `useUpcoming` fallback default to `[]`. Types: `Game.themes` + `Game.playerPerspectives` (required `string[]`); `IgdbSearchResult.themes` + `playerPerspectives`; `IgdbUpcomingRelease.themes` + `playerPerspectives`; `DashboardStats.themes` + `playerPerspectives` (same `{name, count}[]` shape as `genres`). **Backfill script** at `scripts/backfill-game-tags.ts` — re-fetches existing Game rows from IGDB via `getGame(igdbId)`, persists `themes` + `playerPerspectives`; throttled ~3 req/s; resumable (skips rows that already have non-empty values); supports `--dry-run`. Not auto-run; Andrea triggers post-deploy. **Dashboard consumer:** `breakdownMap` aggregations extended in `routes/dashboard.ts` to tally themes + perspectives alongside genres in the same iteration over `aggUserGames` (one extra column on the existing `select`; no new round-trip). Both desktop + mobile breakdown cards now render a 3-tab strip (genre · theme · perspective) bound to a single `breakdownTab` state at the parent. Tabs use `.chip.solid.amber` for active (same pattern as DASH-PR2 `PeriodToggle`); disabled state when a dimension is empty in the user's library. Server route `GET /api/games` already extended with `?genre=` / `?theme=` / `?perspective=` Zod-validated query params + Prisma `array.has` filter assembly — **API surface ready for B-IGDB-3b** to wire client-side chip strips + new primary-lens routes (`/library/by-genre/:slug` etc.) + browse-by panel.
    - **Deferred to B-IGDB-3b (Library consumer):** secondary filter chip strips for genre/theme/perspective on the existing `/library/:status` Status lens; new primary-lens routes `/library/by-genre/:slug` + `/library/by-theme/:slug` + `/library/by-perspective/:slug`; browse-by panel rows on the Library overview surfacing top-N per dimension. The Library secondary-filter system is structurally bigger (interacts with the existing platform filter, sort, find input; needs URL state design + composability rules); shipping it standalone in a follow-up keeps the foundation PR reviewable.
    - **Tests:** +4 frontend in `Dashboard.bento.test.tsx` covering the breakdown 3-tab strip (renders 3 tabs; defaults to genre; clicking theme swaps body to themes; disabled state when perspectives series is empty). Existing backend test mocks for `Game.upsert` use loose `objectContaining` matchers so the new `themes` + `playerPerspectives` fields don't break them; 6 test-factory backfills added in releases + igdbMatch + AddGameModal + RemapGameModal + Dashboard.bento + ReleasesRecentDesktop. **18/18 Dashboard.bento + 112/112 releases + full backend suite — 39 suites / 589 tests pass.** Typecheck + lint + rename-rule + production build all clean.
    - **Bundle delta:** DashboardDesktop chunk 16.21 → 17.45 kB (+0.39 kB gzipped — 3-tab UI logic), DashboardMobile 12.24 → 13.32 kB (+0.34 kB gzipped). Library bundles unchanged (consumer deferred). No `AGENT.md` entry — B-IGDB-3 partial's decisions constrain the IGDB-tag triple specifically.

    **B-IGDB-3b2 landing notes 2026-05-31 (follow-up — primary-lens routes + browse-by panel + change-lens + find-input).** Completes the IGDB-tag-triple workstream. Promotes genre/theme/perspective from secondary filters into PRIMARY lens types (their own route family) and adds the cross-everything affordances on filtered views. **What shipped:** (i) three new routes — `/library/by-genre/:slug`, `/library/by-theme/:slug`, `/library/by-perspective/:slug` — registered in `apps/web/src/App.tsx` as explicit routes (vs. a generic `by-:dim/:slug` capture to keep matching unambiguous + lens-specific error states easy). All four lens types (Status + tag triple) share the same filtered-view UI; the page heading shows lens type as a faint prefix + canonical tag name in accent. (ii) new helpers `apps/web/src/lib/tagSlug.ts` (~30 lines, 11 tests) — `slugifyTag("Role-playing (RPG)") → "role-playing-rpg"`, `findTagBySlug(allTags, slug)` round-trip lookup; slugs not stable identifiers (re-derived from name on every read), acceptable because IGDB tag renames are rare and stale bookmarks 404 to a "tag not found" view. (iii) new backend endpoint `GET /api/games/lens-index` returning `{ genre: TagCount[], theme: TagCount[], perspective: TagCount[] }` — every IGDB-tag value present in the user's library with UserGame counts, sorted count desc + name asc; single Prisma query iterating all the user's UserGames, aggregating client-side per dimension. **Cache-Control: private, max-age=30**. Consumed by both the browse-by panel + the slug→canonical-name resolution on the lens routes. (iv) new client hook `useLensIndex()` (~20 lines) caching the response via SWR + PERF-1 localStorage persistence. (v) `BrowseByPanel` (~110 lines, 8 tests) for mobile + Steam-style sidebar `BrowseByGroups` (~110 lines inline in `Sidebar.tsx`, 7 tests) for desktop. **Andrea-driven mid-PR pivot:** initial PR placed `BrowseByPanel` at the BOTTOM of `/library` overview per the §4.4.2 mockup. Andrea's eyeball: *"I dont see a way to access the different library views ... Take the same approach steam uses when browsing the library."* — rebuilt as a **persistent left-rail navigation** in the Sidebar (always-visible cross-app), with three collapsible groups (genre · theme · perspective). Each group: header `▸ genre 22` (collapsed by default; auto-opens when the active route is on its dimension); click to toggle; expanded shows top-5 + `[show all N →]` toggle to reveal the rest. Click any value → `navigate(/library/by-X/:slug)`. Active value in URL gets `aria-current=page` highlighting. The mobile shell has no sidebar so LibraryMobile still renders the inline `BrowseByPanel` below shelves (one component, two consumer surfaces). Progressive disclosure preserved everywhere: dimensions with zero data hidden entirely. OQ-LIB-7 inline-expand decision still holds — no separate per-dimension index pages at Hoard's scale. (vi) new component `apps/web/src/components/library/ChangeLensPopover.tsx` (~135 lines, 6 tests) — `[change lens ▾]` chip with WAI-ARIA listbox pattern (same shape as FilterPopover); pivots primary lens while preserving intersection. For v1 the pivot rules are: from Status lens, can pivot to a tag lens IF the corresponding `?genre=`/`?theme=`/`?perspective=` is set in URL (status becomes secondary on the new primary). Reverse + tag↔tag pivots ship when their secondary filters land (deferred — status-as-secondary on tag lenses ships when the secondary filter visibility table broadens beyond the tag-triple + platform). Disabled options grey out with "—" hint. (vii) new hook `useLensRoute()` (~40 lines) — single source of truth for parsing the active lens from URL pathname + params; returns `{ type: 'status' | 'genre' | 'theme' | 'perspective' | null, slug: string | null }`. (viii) **LibraryDesktop + LibraryMobile** refactored to be lens-aware: `useGames()` params switch on `lens.type` (status → `?status=`, tag → `?genre=`/`?theme=`/`?perspective=` using `lensCanonical` resolved from `lens-index`); filtered-view branch widened from `if (statusParam)` to `if (filteredEnabled && lens.type)`; secondary FilterPopover dimensions filtered to exclude the PRIMARY one (e.g. on Genre lens, only theme + persp popovers); URL-param reset effect now triggers on `lens.slug` + `lens.type` change instead of `statusParam`. (ix) **find input** on filtered views — `?q=` URL param, client-side filter over the already-loaded games (limit:50000 means full intersection in memory); composes with all other filters; cleared on lens change. (x) **slug-not-resolved guard** — when lens-index loads and the URL slug doesn't match any known tag, renders a "tag not found" view with `[back to library]` instead of an empty grid. (xi) **two-row filter bar** preserved from the FilterPopover workstream — Row 1 anchors `[+ add game]` top-right alongside `find`, `[change lens ▾]`, `sort`; Row 2 holds platform chips + secondary tag popovers. **API contract:** new `LensIndexResponse` type in `@hoard/types`; `api.lensIndex()` client method; cache key `lens-index` (no params — there's nothing to vary on at the user-scoped level besides the JWT). **Tests:** 25 new across the workstream (11 tagSlug + 8 BrowseByPanel + 6 ChangeLensPopover) plus 4 backend tests for `/api/games/lens-index` (aggregation correctness, count-tie name-asc tiebreak, empty-library edge, Cache-Control header). **594 backend tests pass** (+5 vs the 589 baseline). Targeted vitest 47/47. Typecheck + lint + rename-rule + production build all clean. **Bundle delta:** LibraryDesktop chunk 17.15 → 20.63 kB (+0.85 kB gzipped — lens detection + change-lens + browse-by + find input + slug resolution); LibraryMobile 15.47 → 18.83 kB (+0.78 kB gzipped — same surfaces on mobile shell); initial JS index +0.30 kB. **Sticky properties:** (1) **`useLensRoute()` is the single source of truth** for lens detection — any future lens type (Collection / Developer / Publisher / Series / Rating per §4.4.1) extends the `LensType` union + adds a `/library/by-<X>/:slug` route + a branch in the hook's pathname parsing; everything else (filtered view, change-lens, browse-by) inherits the new lens for free. (2) **Browse-by panel** generalizes via `LensIndexResponse`'s shape — adding Collection / Developer / Publisher rows means extending the API response with `collection: TagCount[]` etc. + a new `<Row>` in `BrowseByPanel.tsx`. (3) **Slug round-trip** — `slugifyTag` + `findTagBySlug` are dimension-agnostic; reuse for any future reference-data lens (Developer / Publisher / Series).

    **B-IGDB-3b1 landing notes 2026-05-30 (follow-up — chip strips on Library Status lens).** Wires the IGDB-tag triple foundation into the Library secondary filter UI. Server filter surface was already live (`?genre=&theme=&perspective=`) from the foundation PR; this is a pure-frontend follow-up. **What shipped:** new `apps/web/src/lib/pickTopTags.ts` (~50 lines) — `pickTopTags(games, dimension, cap)` returns the top-N distinct tags ordered by occurrence count desc, ties broken by name asc (deterministic); `filterByTag(games, dimension, value)` is the composable client-side predicate. Both helpers are dimension-agnostic across genre/theme/perspective. **LibraryDesktop + LibraryMobile** — three new chip rows (genre · theme · persp) below the existing platform row on the `/library/:status` filtered view. Each row only renders when the loaded games carry that tag (progressive disclosure pre-backfill). Chip values derived from the FULL shelf pre-filter, so the user sees all options even after narrowing on another dimension; if the active filter is a tag not in the top-N (edge case after backfill changes the distribution), it's prepended so the user can still see + un-toggle it. URL-state-resident via `useSearchParams`: `?genre=` / `?theme=` / `?perspective=` — composable with each other and the existing `?sort=` param; cleared on `statusParam` change (same React Router 6 reuse-across-route-params reset as `platFilter`). `applyFilters` extended to compose three new `filterByTag` calls after the platform filter, before sort. **Tests:** 11 unit tests in `apps/web/src/lib/__tests__/pickTopTags.test.ts` pinning count-desc + name-asc ordering, cap, per-dimension reads, empty edge cases, null/undefined no-op, and dimension-specific filtering. **Bundle delta:** LibraryDesktop 16.09 → 17.29 kB (+0.38 kB gzipped), LibraryMobile 14.35 → 15.53 kB (+0.38 kB gzipped). Modest. Typecheck + lint + rename-rule + production build all clean. Targeted vitest 44/44 (11 pickTopTags + 1 LibraryDesktop + 14 api-invalidation + 18 Dashboard.bento). **Still deferred to B-IGDB-3b2:** primary-lens routes `/library/by-genre/:slug` + `/library/by-theme/:slug` + `/library/by-perspective/:slug` + browse-by panel on Library overview. These need URL design (e.g. `/library/by-genre/RPG?perspective=First-person`) + Library page IA changes; structurally distinct from chip strips. **Sticky property for B-IGDB-3b2 implementor:** the helper file is dimension-agnostic across genre/theme/perspective so a future Collection / Developer / Publisher dimension (B-Stash-4, B-Stash-5) can use the same shape by extending the `TagDimension` union. The chip-row JSX in both Library components is also pure repeat-for-each-dimension; new lenses thread in without restructuring.
...
- **GD-PR1** **(Done 2026-05-31)** Route migration to `/game/:igdbId` (with cuid → IGDB 301 via the new `/api/games/usergame/:id/igdb-id` resolver) + state-aware shell rendering + S1 (released-not-owned) basics: cover, meta, `[+ add to library]` (dominant) + `[+ wishlist]` (alt), description, ITAD price-offers card, lateral-nav + Collections + events placeholders. Wishlist UserGames also route to S1 with adapted CTAs (`[+ acquire]` / `[- remove from wishlist]`) because the legacy S3 surface didn't fit them.
- **GD-PR2** **(Done 2026-05-31)** S2 (upcoming-not-owned) with giant `T-N` countdown promoted to dominant page element + per-region × per-platform release-dates panel (collapsible) + Steam/GOG/PSN preorder deep-links (Xbox/Epic/Nintendo deferred per OQ-GD-14) + IGDB screenshots gallery (click → full-res new tab) + IGDB videos rail (YouTube link-out; embed deferred to polish PR per OQ-GD-14 CSP) + price-offers card (MSRP fine per Andrea) + latest-news placeholder. HLTB row *absent* from S2 per spec (§3.3). Wishlist+future-release UserGames route to S2 with adapted CTAs.
...
- **DEALS-PR1 — Foundation (digital, locale-aware).** **Shipped 2026-05-31.** See landing notes below.
    ...
    **DEALS-PR1 landing notes 2026-05-31.** First entry in the DEALS-series. **What shipped end-to-end:** (i) Schema — `User.marketCode String?` + `Deal { id, gameId, shopId, shopName, isReseller, currentPrice, originalPrice, currency, discountPct, dealUrl, voucher?, expiresAt?, storeLow?, isHistoricalLow, isTrendingDown, fetchedAt }` + `PriceSnapshot { id, gameId, shopId, price, currency, snapshotAt }`; migration `20260531180000_deals_foundation` applied to Supabase via the documented `prisma db execute` + Node `$executeRaw` migration-record fallback. (ii) ITAD client — `apps/api/src/services/itad.ts`: `isItadConfigured()` gates calls on `ITAD_API_KEY` env var (non-Andrea environments without the key return null deals gracefully — no crash); `lookupItadIdBySteamAppId(appId)` via `/lookup/id/shop/61/v1` for stable Steam-keyed lookup; `lookupItadIdsByTitles(titles)` via `/games/lookup/v1` bulk POST for non-Steam-keyed games; `getPricesForGames(ids, country)` via `/games/prices/v3` POST (chunked 200-per-batch). All endpoints throw `ItadClientError` on misconfiguration/network failure; caller (orchestrator) catches + logs + continues. (iii) Storefront classification — `apps/api/src/services/deals/storefronts.ts`: case-insensitive name match against two locked sets (Tier-1 first-party — Steam/GOG/Epic/Humble Store/Battle.net/itch.io/PSN/Xbox/Nintendo eShop; Tier-2 reseller allow-list — Humble Bundle/Instant Gaming/GMG/Kinguin/CDKeys per OQ-DEALS-9). Anything off-list (G2A/Eneba/Fanatical/GamesPlanet) returns `'excluded'` and gets dropped at sync time before persisting. 22 unit tests pin every classification row + the expansion-candidate-not-yet-allow-listed cases. (iv) Affiliate router — `apps/api/src/services/deals/affiliate.ts`: per-reseller env-var-keyed affiliate IDs (HUMBLE_AFFILIATE_ID / INSTANT_GAMING_AFFILIATE_ID / GMG_AFFILIATE_ID / KINGUIN_AFFILIATE_ID / CDKEYS_AFFILIATE_ID / GOG_AFFILIATE_ID / ITCHIO_AFFILIATE_ID). When the env var is unset (Andrea hasn't signed up yet), URL passes through unrewritten — identical UX, just no cost-recovery routing per OQ-DEALS-5. Per-reseller param names hardcoded (`partner` for Humble, `igr` for Instant Gaming, `mw_aref` for GMG/CDKeys, `tap_s` for Kinguin, `as_aid` for GOG, `ref` for itch.io). Storefronts with no affiliate program (Steam/PSN/Xbox/Nintendo/Epic) have no mapping — links go direct. 17 unit tests cover every reseller + idempotency + URL-parse-failure graceful degradation. (v) Sync orchestrator — `apps/api/src/services/deals/syncDeals.ts`: resolves ITAD IDs (steamAppId-first, title-lookup fallback; caches result on `Game.metadata.itadId` so future syncs skip the lookup); fetches prices in 200-per-batch chunks at ~3 req/s; classifies + filters via `storefronts.ts`; upserts Deal rows per (gameId, shopId); writes daily PriceSnapshot for every shop returned (used by trend detection); deletes stale Deal rows for shops no longer on sale. **Trending-down detection:** at sync time, after each snapshot insert, queries last 30 days of snapshots for that (game, shop) and sets `Deal.isTrendingDown` true when ≥2 distinct drops. Thresholds tunable via `TRENDING_DOWN_DROPS` / `TRENDING_DOWN_DAYS` env vars per OQ-DEALS-13. (vi) Routes — `GET /api/deals` returns `{ topWishlistDeal, wishlistDeals, broaderFeed, marketCode, lastSyncedAt }`. CM12 + already-owned exclusion applied at query time: a game owned via UserGame status≠Wishlist + zero wishlistedPlatforms → skipped from broader feed (the user has it; deals are noise). A game owned on PSN but wishlisted on Switch (CM12 per-platform wishlist) → still appears in wishlistDeals. Deal URLs affiliate-routed BEFORE the response leaves the server — frontend never sees raw URLs. `POST /api/admin/deals/refresh` triggers `syncAllDeals()` for the admin manual button. `PATCH /api/auth/me` extended with `marketCode: string | null` (ISO 3166-1 alpha-2, regex-validated; null clears). (vii) Dashboard wiring — `DashboardResponse.wishlistDealsCount: number` field; `prisma.deal.count` query for the user's wishlist game IDs. `AlertsStrip` extended with a second chip kind (amber bordered `// X wishlist games on sale`) that navigates to `/deals`. Both chips render conditionally; both null returns `null` (no strip). (viii) Frontend — new `apps/web/src/components/screens/DealsDesktop.tsx` (~190 lines) + `DealsMobile.tsx` (~110 lines). Desktop: hero card (cover-left, 240×135 landscape, metadata-right with -X% green, prices, chips, `[buy on Shop →]`); list rows (60×34 cover, title + storefront subline with inline chips, discount %, price, expiry, `buy →`). Mobile: compressed per OQ-DEALS-4 — hero collapses to a regular row; cards drop the storefront chip to a subline; pull-to-refresh wired. New `useDeals()` SWR hook + `api.deals()` client method. Sidebar `Deals` nav entry (`tag` glyph — new icon path added; positioned between Releases and Settings). MobileTabBar widened from 4 to 5 tabs (added Deals between Soon and Me; CSS grid `repeat(5, 1fr)`). (ix) Settings → Account market picker — new `<select>` row with 10 ISO 3166-1 options (AT/DE/IT/FR/ES/GB/US/CA/AU/JP + clear); `saveMarket(next)` calls `api.updateMe({ marketCode })`; same auto-save + green `// saved` toast as the existing email/name fields. **Types — `@hoard/types`:** new `DealRow` + `DealsResponse` interfaces; `AuthUser.marketCode` (required `string | null`); `PatchMeBody.marketCode?: string | null`; `DashboardResponse.wishlistDealsCount: number`. **Tests:** 39 new backend (22 storefronts + 17 affiliate); existing dashboard tests updated to mock `prisma.deal.count` (default 0) + the new `gameId`/`wishlistedPlatforms` columns on the `aggUserGames` select; 9 test fixtures backfilled with `marketCode: null` + `wishlistDealsCount: 0`. **642/642 backend tests pass** (+48 vs the 594 pre-DEALS baseline). 61/61 affected web tests pass. Typecheck + lint + rename-rule + production build all clean. **Bundle delta:** new DealsDesktop chunk 7.68 kB / 2.33 kB gzipped + DealsMobile 3.90 kB / 1.61 kB gzipped (lazy-loaded); DashboardDesktop +0.05 kB gzipped (wishlistDealsCount chip wire); initial JS index 260.74 → 261.34 kB (+0.16 kB gzipped, type-only). **Pre-deploy ops to set on Railway:** `ITAD_API_KEY` (Andrea registering in parallel — until set, /deals renders empty); affiliate IDs per-reseller (optional; when unset, buy-links go direct). **Sticky properties for future DEALS PRs:** (1) `Deal.isReseller` distinguishes Tier-1 from Tier-2 at query level — frontend filter chips by source-type read this column directly. (2) `PriceSnapshot` is the source of truth for trend detection; the `isTrendingDown` flag on Deal is the cached output. Prune snapshots older than 90 days in a future cron. (3) Affiliate router is `routeAffiliateUrl(shopName, url)` — single function, called server-side BEFORE the response leaves. Adding a new reseller = 2 lines (one to `AFFILIATE_ENV_VARS`, one to `AFFILIATE_PARAM_NAMES`). (4) Sale-event grouping (DEALS-PR2) and physical deals (DEALS-PR3) layer on top of this foundation without schema changes to the Deal/PriceSnapshot tables.
