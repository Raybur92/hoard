...
| B-Art-1 — smarter IGDB hero-image picker (cover-dup veto + portrait skip + array-order tiebreak) | Done (2026-06-01) | Two iterations after Andrea's eyeball — v1 single-pool scorer pulled to screenshots; v2 two-stage with in-pool scoring still reordered artworks; v3 dropped in-pool scoring entirely (IGDB array order wins inside stage 1). Final picker: first non-portrait non-cover-dup artwork, fall through to screenshot. 34 swaps applied to live DB via `scripts/rescore-game-hero-image.ts`. Companion `scripts/compare-hero-rescore.ts` for visual spot-checks. Lesson: algorithmic scoring of editorial assets returns diminishing trust quickly. |
| GD-PR4 — S4 (Completed) GameDetail surface + archivist relic (OQ-GD-13) | Done (2026-06-01) | Final piece of the GD-series. Plan at `docs/GD_PR4_PLAN.md`. **GD-PR4a (`1727b84`):** `Game.relicDitherSvg` schema column; sharp-based shape-dither render service with per-cell radial-wave animation-delay baked in at stage-3 absolute offset; sigil classifier (3-dim: GENRE/THEME/PERSPECTIVE clusters, 24 unique sigils); GameDetail response extension; self-healing invalidation via `<!-- src=URL;fmt=2 -->` XML comment embedded in each SVG (mismatch with current heroImageUrl triggers re-render — no per-writer cache-bust across the 6+ Game.upsert call sites); backfill script. 25 new unit tests. Backfilled 87/87 Completed games avg ~775KB per SVG. **GD-PR4b (`9971b05` + polish `d01c490`):** **architectural reversal mid-PR after Andrea's eyeball** — initial S4-as-default-Completed-surface put the relic ahead of the editing flow (wrong order). Revised: Completed games stay on legacy GameDetail; amber `[★ see relic]` chip opens a full-screen **read-only overlay** with the relic. Auto-opens once on first status-flip to Completed (consecration moment). RelicCard gained a `readonly` prop swap. 5-stage CSS animation choreography (~2.4s; `prefers-reduced-motion` collapses to 200ms fade; first-visit-only via `hoard:relic-consecrated:${userGameId}` localStorage key). RelicOverlay with focus trap, Esc/scrim close, body-scroll lock; fetches via existing `useGameByIgdb` hook (cache-hit when dispatcher already loaded). Cache version bumped v4→v5. 20 frontend tests. **GD-PR4c deferred** — polish (animation timing tuning, mobile layout tweaks, print/share); only if cohort signal demands. |
...
| R2 — remap durability (fold source Game's platform-side IDs into target) | Done (2026-06-01) | Surgical follow-up to GD-PR3 after Andrea reported the same wrong-game match resurfacing 10+ times. **What shipped:** new `computeRemapFold(source, target)` helper + `PLATFORM_ID_FIELDS` const in `apps/api/src/routes/games.ts`. Rule: move a field's value from source → target IFF source has non-null AND target has null; never overwrite. Both remap branches (non-collision + collision-merge) wrap the fold + UserGame update in `$transaction` so clear-source happens BEFORE set-target to release `@unique` constraints. +3 new R2 tests; existing remap tests updated to mock `$transaction`. **702/702 backend pass** (+3 vs GD-PR3's 699). **R1 deferred** — the bigger durability layer (sync-time pre-check by user's existing UserGame for platform-side ID) touches every sync writer; R2 should cover most cases without it. |
