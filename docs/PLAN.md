...
| GD-PR1 — GameDetail v2 route migration + state-aware shell + S1 (released-not-owned) | In progress (started 2026-05-31) | First PR of the GD-series per `docs/PAGES_PLAN.md` §3.6. Establishes the 4-state architecture (S1/S2/S3/S4 per OQ-GD-12). Route migrates from `/game/:userGameId` to `/game/:igdbId` (cuid form 301s during a transition window per OQ-GD-1). State detector switches on UserGame existence + release date: no UserGame + released → S1; no UserGame + future release → S2; UserGame status≠Completed → S3; UserGame status=Completed → S4; UserGame status=Wishlist + future release → S2; UserGame status=Wishlist + past/null release → S3 (library-citizen framing per OQ-GD-12). **GD-PR1 scope:** S1 shipped fully — cover + meta (year, platforms, genres) + `[+ add to library]` (dominant) + `[+ wishlist]` (alt) + description + ITAD price-offers card consuming the DEALS-PR1 pipeline per §8.6 sequencing + lateral-nav / Collections / events placeholders. S2/S3/S4 fall back to today's existing GameDetail rendering as the transition strategy (filled out by GD-PR2/PR3/PR4). |
