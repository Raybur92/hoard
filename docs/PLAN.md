...
| Post-8 — Interaction debt audit + PR plan | In progress | Drafted in `docs/INTERACTION_DEBT_PLAN.md` 2026-05-06. **PR D done 2026-05-06** — layered HLTB fallback. Coverage 34.2% → 63.1%. **PR A done 2026-05-06** — all 9 items: mobile back default to navigate(-1) + 44pt hit areas, body overscroll lock killing the wiggle, library-only search input + `/` shortcut, Cmd-K confirmed wired, Library shelves view simplified (no view-mode/sort/plat-filter), 4 Settings stubs become ComingSoonPanel, Account visibility/sessions get V2 markers, HLTB extras+completionist on mobile receipt, Dashboard CTAs/now-playing buttons wired or removed, GameDetail HLTB chip deep-links via `Game.hltbId`, GameDetailDesktop gets a back chip, Upcoming month tabs filter featured + agenda. 125 API + 69 web tests pass. **PR B done 2026-05-06** — real `?scope=wishlist` on `/api/igdb/upcoming` (DB read, no hype filter); wishlist-toggle persistence fix captures `releaseDate` / `platforms` / `synopsis` / `hype` / `category` / `releaseDateCategory` (migration `20260506160000_wishlist_release_category` adds the missing `category` column); `scripts/backfill-wishlist-fields.ts` updated 7/7 existing impoverished rows; UpcomingScope widened to 3 real chips with honest wishlist count. **PR C** unchanged. Two systematic audits captured: (a) **data-flow audit** — Source → Schema → UI map per platform; surfaces orphaned `HypeBars` primitive, dead `Game.metadata` field, Path-A vs Path-B Upcoming persistence asymmetry that drops 5+ fields on wishlist toggle. (b) **interaction audit** — every `<onClick>` traced; ~20+ controls look active but do nothing (search bar, view-mode chips, sync-all, wipe-library, mobile back buttons across Settings + PlatformDetail, Upcoming month tabs, etc.). 14 decisions locked for PR A through D. **Hot fixes already shipped (see row above).** **PR D** (HLTB diagnostic) — runs first, output decides scope of follow-up. **PR A** (interaction debt) — search input + Cmd-K + `/` shortcuts, mobile-shell wiggle fix, mobile back buttons + icon-button sweep per Apple HIG, Settings stubs as "coming soon" panels, library shelves view simplification, HLTB extras+completionist on mobile, bundled cleanups. **PR B** (real wishlist scope server-side + Path-B persistence fix). **PR C** (sync-all + wipe-library with proper feedback). |
