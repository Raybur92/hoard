...
| E2E suite restoration (E-series, OPEN, PARALLEL) | Step 3 Done (2026-05-10), implementation in flight | Plan at `docs/E2E_RESTORATION_PLAN.md`. Triggered by I4's deferral note: existing E2E suite was broken since I1's seed-andrea pre-step deletion. Strategy decision **locked 2026-05-09 (Option F hybrid B+E)** with five Andrea-led refinements (ratio ~30/70 mocked/real-DB, data-parity ≠ schema-parity, a11y false-positive fix folded into E1, snapshot baseline regen, naming convention by isolation level). E1 commits 1+2 landed via parallel session: **`f40ca30`** infra (test-DB keepalive Action + migrate-status CI gate + `.env.test.example`) and **`440b76c`** seed-e2e.ts + Playwright config wiring `DATABASE_URL_TEST`. E1 commit 3 (global auth fixture + spec rewrites + reclassification) in progress in working tree. |
