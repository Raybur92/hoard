...
| Closed-beta invite codes (I-series, IN PROGRESS) | I1–I4 Done (2026-05-08), 3 smoke-test followups (2026-05-09), I5–I6 Pending | Spec at `docs/Hoard_invite_codes.md`, plan at `docs/INVITE_CODES_PLAN.md`. Six PRs (I1 → I6). **I1** schema + migration (commit `6856f10`) — pure additive, three real users backfilled to ACTIVE, Andrea flipped to isAdmin=true. **I2** closed-beta gate middleware + redeem-invite + request-access endpoints (commit `702f493`) — `requireActive` on every gated route, two-tier rate limit, atomic redemption via `$transaction` with `WHERE usedById IS NULL` predicate. **I3** admin endpoints + helpers (commits `4f4ba3f` + `6acdf69`) — four routes under `/api/admin/*`, `requireAdmin` returns 404 not 403, shared `displayIdentity` helper preferring `User.name` over Steam-id fallback. **I4** welcome screen + RequireActive + safeNext open-redirect defense (commit `683b05c`) plus form-level non-empty constraint (`e8ca975`). **Smoke-test followups** (`386059d`, `5024234`, `9051b36`): pre-existing register-bounce bug fixed via `setUser`-before-`navigate` pattern in LoginScreen; status-aware navigation + `safeNext` for `?next=` preservation; `RequireAuth` redirected to `/login?next=<encoded>` instead of router state (the bit that closed the deep-link deferral honestly). Architectural decisions mirrored as #34–#38 in `AGENT.md`. **I5** (admin page + sidebar entry, desktop-only, mobile fallback) PENDING Andrea's smoke #3 retry against `9051b36`. **I6** (rollout + first real codes + doc closeouts) PENDING after I5. **Test counts at last commit: 274 API + 251 web pass; lint + typecheck clean.** |
| E2E suite restoration (E-series, OPEN, PARALLEL) | Step 1 Done (2026-05-09), Step 2 OPEN (strategy decision awaits Andrea) | Plan at `docs/E2E_RESTORATION_PLAN.md`. Triggered by I4's deferral note: existing E2E suite has been broken since I1's seed-andrea pre-step deletion in a deceptive way — `screens.spec.ts` (38 tests) all fail with content-not-found timeouts because the auth chain redirects to `/login`; `a11y.spec.ts` (12 tests) "passes" because axe-core scans `/login` (false-positive coverage). Diagnostic captured in §1 of the plan. Five strategy options laid out with tradeoffs across cost / CI complexity / schema parity / secrets management / dev ergonomics: (A) point DEV_USER_ID at Andrea's real row (disqualified — fragile + prod-coupled), (B) provision dormant `hoard-test` Supabase project (recommended; ~half-day upfront), (C) single project + separate `test` schema (cheaper but wrong-default-schema risk hits prod), (D) mint+reap in prod DB (disqualified per Andrea's standing call from I4), (E) `page.route(...)` API mocks (fast but defeats E2E for content tests), (F) hybrid B+E (recommended starting point — splits the upfront cost). **Held at strategy decision; no test code drafted yet.** Surfaces for Andrea's input when I5 stops per hard rule 10. |
