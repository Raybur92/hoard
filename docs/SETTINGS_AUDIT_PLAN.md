...
| PR B — Platform sync log | Done (2026-05-08) | Schema migration `20260508130000_platform_log` adds `LogLevel` enum + `PlatformLog` table with two composite indexes for cursor pagination. New `apps/api/src/services/platformLog.ts` with the `logPlatform()` helper (failures swallowed so logging never breaks a sync). Write hooks landed in `routes/platforms.ts` at every sync touchpoint: `sync.started`, `library.imported`, `library.unsupported` (XB/GG), `trophies.applied` / `trophies.failed` (PSN), `wishlist.imported` / `wishlist.failed` (ST), `achievements.applied` / `achievements.failed` (ST), `sync.ok` (with duration), `sync.error`. Cursor-paginated `GET /api/platforms/:code/log?cursor=` endpoint, 50 entries/page, ordered `(createdAt desc, id desc)` for stable order across same-millisecond writes. New shared `PlatformLogTab` component reintroduces the Log tab on desktop + mobile in terminal aesthetic — `[YYYY-MM-DD HH:MM:SS] level · message`, color-coded by level (info=green, warn=amber, error=red), `[load more]` button for pagination, "no activity yet" empty state. 8 new tests (3 logPlatform helper + 4 read endpoint + 1 mock fix on existing). **226 API + 196 web tests pass.** |
